#tp to hole one
tp @s[tag=hole1,tag=normal_mode] -92 -58 42
tp @s[tag=hole1,tag=easy_mode] -126 -58 16

#tp to hole two
tp @s[tag=hole2,tag=normal_mode] -246 -58 -30
tp @s[tag=hole2,tag=easy_mode] -292 -58 26

#tp to hole three
tp @s[tag=hole3,tag=normal_mode] -273 -56 154
tp @s[tag=hole3,tag=easy_mode] -235 -56 179

#tp to hole four
tp @s[tag=hole4,tag=normal_mode] -206 -56 206
tp @s[tag=hole4,tag=easy_mode] -145 -56 161
